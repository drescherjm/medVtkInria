...
Program:   vtkINRIA3D
Module:    $Id: vtkImageViewCollection.cxx 917 2008-08-27 10:37:34Z filus $
Language:  C++
...
//       if (item2d)
//       {
// 	vtkActor* actor = item->AddDataSet (a2d->GetSlicePlane());
// 	if (actor)
// 	  actor->SetVisibility (this->ShowAxes);
// 	actor = a->AddDataSet (item2d->GetSlicePlane());
// 	if(actor)
// 	  actor->SetVisibility (this->ShowAxes);
//       }
//       else if (item3d)
// 	item3d->Add2DPhantom(a->GetImageActor());
...
// 	a3d->Add2DPhantom(item->GetImageActor());
//       item = this->GetNextItem();
//     }
  }
...
void vtkImageViewCollection::SyncRender(vtkImageView* caller)
{
  this->InitTraversal();
  vtkImageView* item = this->GetNextItem();
  //vtkImageView3D* item3d;
  while(item)
  {
    if (item!=caller)
    {
      /*
      item3d = vtkImageView3D::SafeDownCast (item);
					    ...
					    void *callData)
{
  ...
  while(v)
  {
    if (isi == v->GetInteractorStyle())
      viewer = vtkImageView2D::SafeDownCast (v);
  ...
  {
    this->Collection->SyncSetCurrentPoint (viewer->GetCurrentPoint(), viewer);
    this->Collection->SyncRender(viewer);
  }

  // Adjust the window level here
  if (event == vtkCommand::WindowLevelEvent)
  {
    this->Collection->SyncSetColorWindow(viewer->GetColorWindow(), viewer);
    this->Collection->SyncSetColorLevel(viewer->GetColorLevel(), viewer);
    this->Collection->SyncRender(viewer);
  }
  ...
  if (event == vtkImageView2DCommand::DefaultMoveEvent)
  {
    ...
    this->Collection->SyncSetCurrentPoint(position, viewer);
  ...
  if (event == vtkImageView2DCommand::RequestedPositionEvent)
  {
    ...
    this->Collection->SyncSetCurrentPoint(position);
    this->Collection->SyncRender(viewer);
    viewer->Render();
  }  
