		...
		*/
		...
		if (retVal.empty()) {
			...
			if (valueImageLaterality.IsValid()) {
				...
				valueImageLaterality = m_pPrivate->meta->Get(DC::Laterality);
				if (valueImageLaterality.IsValid()) {
					retVal = valueImageLaterality.AsString();
				}
			}
	...
	*/
	...
	if (isMultiframeDicom()) {
		...
		if (value.IsValid()) {
			...
			value = m_pPrivate->meta->Get(
				vtkDICOMTagPath(DC::PerFrameFunctionalGroupsSequence, 0,
					DC::PlaneOrientationSequence, 0,
					DC::ImageOrientationPatient)
			);
			...
			if (value.IsValid()) {
				...
				value = m_pPrivate->meta->Get(DC::ImageOrientationPatient);
